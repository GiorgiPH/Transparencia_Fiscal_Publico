...
export const useCatalogos = () => {
  ...
  const cargarCatalogoHijos = useCallback(async (catalogoId: number, catalogoNombre: string, nivel: number) => {
    setLoading(true);
    setError(null);
  ...
  const navegarABreadcrumb = useCallback(async (breadcrumb: BreadcrumbItem) => {
    if (breadcrumb.nivel === 0) {
      ...
      await cargarCatalogoHijos(breadcrumb.id, breadcrumb.nombre, breadcrumb.nivel);
    }
  ...
  const navegarAtras = useCallback(async () => {
    ...
    if (breadcrumbsAnteriores.length === 0) {
      ...
      await cargarCatalogoHijos(ultimoBreadcrumb.id, ultimoBreadcrumb.nombre, ultimoBreadcrumb.nivel);
    }
